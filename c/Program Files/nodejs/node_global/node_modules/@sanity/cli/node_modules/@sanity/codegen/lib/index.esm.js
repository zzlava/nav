...
function parseSourceFile(source, filename, babelOptions) {
  const result = parse$1(source, __spreadProps$1(__spreadValues$2({}, babelOptions), {
    filename
  }));
  if (!result)
...
function resolveExpression({
  ...
  if (t.isVariableDeclarator(node)) {
    const init = (_c = node.init) != null ? _c : t.isAssignmentPattern(node.id) && node.id.right;
    if (!init)
...
const require2 = createRequire(__filename), groqTagName = "groq", defineQueryFunctionName = "defineQuery", groqModuleName = "groq", nextSanityModuleName = "next-sanity", ignoreValue = "@sanity-typegen-ignore";
    ...
    try {
      for (var iter = __forAwait(stream), more, temp, error; more = !(temp = yield new __await(iter.next())).done; more = !1) {
        ...
        if (typeof filename == "string") {
          ...
          try {
            const source = yield new __await(fs$1.readFile(filename, "utf8")), queries = findQueriesInSource(source, filename, babelOptions, resolver);
...
function registerBabel(babelOptions) {
  ...
  register(__spreadProps(__spreadValues({}, options), { extensions: [".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"] }));
}
