...
function parseSourceFile(source, filename, babelOptions) {
  const result = core.parse(source, __spreadProps$1(__spreadValues$2({}, babelOptions), {
    filename
  }));
  if (!result)
...
function resolveExpression({
  ...
  if (t__namespace.isVariableDeclarator(node)) {
    const init = (_c = node.init) != null ? _c : t__namespace.isAssignmentPattern(node.id) && node.id.right;
    if (!init)
  ...
  throw new Error(
    `Unsupported expression type: ${node.type} in ${filename}:${(_d = node.loc) == null ? void 0 : _d.start.line}:${(_e = node.loc) == null ? void 0 : _e.start.column}`
  );
}
...
const require2 = node_module.createRequire(__filename), groqTagName = "groq", defineQueryFunctionName = "defineQuery", groqModuleName = "groq", nextSanityModuleName = "next-sanity", ignoreValue = "@sanity-typegen-ignore";
    ...
    VariableDeclarator(path2) {
      ...
      if (t__namespace.isIdentifier(node.id) && (isGroqTemplateTag || isDefineQueryCall)) {
        ...
        const queryName = `${node.id.name}`, queryResult = resolveExpression({
          ...
          start: __spreadValues$1({}, (_a = node.loc) == null ? void 0 : _a.start),
          end: __spreadValues$1({}, (_b = node.loc) == null ? void 0 : _b.end)
        } : {};
...
}, __asyncGenerator = (__this, __arguments, generator2) => {
  var resume = (k, v, yes, no) => {
    try {
      var x = generator2[k](v), isAwait = (v = x.value) instanceof __await, done = x.done;
      Promise.resolve(isAwait ? v[0] : v).then((y) => isAwait ? resume(k === "return" ? k : "next", v[1] ? { done: y.done, value: y.value } : y, yes, no) : yes({ value: y, done })).catch((e) => resume("throw", e, yes, no));
    } catch (e) {
      no(e);
    }
  }, method = (k) => it[k] = (x) => new Promise((yes, no) => resume(k, x, yes, no)), it = {};
  return generator2 = generator2.apply(__this, __arguments), it[__knownSymbol("asyncIterator")] = () => it, method("next"), method("throw"), method("return"), it;
...
function findQueriesInPath(_0) {
  return __asyncGenerator(this, arguments, function* ({
    ...
    try {
      for (var iter = __forAwait(stream), more, temp, error; more = !(temp = yield new __await(iter.next())).done; more = !1) {
        const filename = temp.value;
        if (typeof filename == "string") {
          debug(`Found file "${filename}"`);
          try {
            const source = yield new __await(fs__default$1.default.readFile(filename, "utf8")), queries = findQueriesInSource(source, filename, babelOptions, resolver);
            ...
            debug(`Error in file "${filename}"`, error2), yield { type: "error", error: error2, filename };
          }
        }
      }
    } catch (temp2) {
...
var __defProp$1 = Object.defineProperty, __defProps = Object.defineProperties, __getOwnPropDescs = Object.getOwnPropertyDescriptors, __getOwnPropSymbols = Object.getOwnPropertySymbols, __hasOwnProp = Object.prototype.hasOwnProperty, __propIsEnum = Object.prototype.propertyIsEnumerable, __defNormalProp$1 = (obj, key, value) => key in obj ? __defProp$1(obj, key, { enumerable: !0, configurable: !0, writable: !0, value }) : obj[key] = value, __spreadValues = (a, b) => {
  for (var prop in b || (b = {}))
    __hasOwnProp.call(b, prop) && __defNormalProp$1(a, prop, b[prop]);
  if (__getOwnPropSymbols)
    for (var prop of __getOwnPropSymbols(b))
      __propIsEnum.call(b, prop) && __defNormalProp$1(a, prop, b[prop]);
  return a;
}, __spreadProps = (a, b) => __defProps(a, __getOwnPropDescs(b));
function registerBabel(babelOptions) {
  const options = babelOptions || getBabelConfig();
  register__default.default(__spreadProps(__spreadValues({}, options), { extensions: [".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"] }));
}
